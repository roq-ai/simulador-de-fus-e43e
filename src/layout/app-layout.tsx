...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiCodepen } from 'react-icons/fi';

import { DesenvolvedoraInterface } from 'interfaces/desenvolvedora';
import { getDesenvolvedoras } from 'apiSdk/desenvolvedoras';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DesenvolvedoraInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/desenvolvedoras` : null),
    () => getDesenvolvedoras({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/desenvolvedoras/view/${data?.[0]?.id}` : '/desenvolvedoras',
      entity: 'desenvolvedora',
      service: AccessServiceEnum.PROJECT,
      icon: FiCodepen,
    },
    { name: 'Cars', path: '/cars', entity: 'car', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Games', path: '/games', entity: 'game', service: AccessServiceEnum.PROJECT, icon: FiBox },
